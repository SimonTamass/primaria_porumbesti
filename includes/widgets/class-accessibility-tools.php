...
final class Accessibility_Tools extends Base {
	...
	protected function render(): void {
		...
		<div class="porumbesti-a11y porumbesti-a11y-<?php echo esc_attr( $s['position'] ); ?>">
			<div id="<?php echo esc_attr( $panel_id ); ?>" class="porumbesti-a11y-panel" role="dialog" aria-labelledby="<?php echo esc_attr( $title_id ); ?>" hidden><h2 id="<?php echo esc_attr( $title_id ); ?>"><?php echo esc_html( $s['title'] ); ?></h2><div class="porumbesti-a11y-row"><span><?php echo esc_html( $s['text_size_label'] ); ?></span><span><button data-porumbesti-scale="down">−</button><strong data-porumbesti-scale-label>100%</strong><button data-porumbesti-scale="up">+</button></span></div><div class="porumbesti-a11y-row"><span><?php echo esc_html( $s['contrast_label'] ); ?></span><button class="porumbesti-switch" data-porumbesti-a11y="contrast" aria-pressed="false"><i></i></button></div><div class="porumbesti-a11y-row"><span><?php echo esc_html( $s['grayscale_label'] ); ?></span><button class="porumbesti-switch" data-porumbesti-a11y="grayscale" aria-pressed="false"><i></i></button></div><div class="porumbesti-a11y-row"><span><?php echo esc_html( $s['underline_label'] ); ?></span><button class="porumbesti-switch" data-porumbesti-a11y="underline" aria-pressed="false"><i></i></button></div><button class="porumbesti-button porumbesti-button-soft" data-porumbesti-reset><?php echo esc_html( $s['reset_label'] ); ?></button></div>
			<div class="porumbesti-floating"><button data-porumbesti-a11y-toggle aria-expanded="false" aria-controls="<?php echo esc_attr( $panel_id ); ?>" aria-label="<?php echo esc_attr( $s['options_label'] ); ?>"><svg class="porumbesti-a11y-toggle-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="4.5" r="2"></circle><path d="M5 8.5c4.5 1.5 9.5 1.5 14 0M12 10v10M8.5 20 12 14l3.5 6"></path></svg></button><button data-porumbesti-top aria-label="<?php echo esc_attr( $s['back_to_top_label'] ); ?>">↑</button></div>
		</div>
		<?php
	}
}

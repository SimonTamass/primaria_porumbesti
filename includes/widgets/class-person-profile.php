...
final class Person_Profile extends Base {
	...
	protected function render(): void { $s = $this->get_settings_for_display(); ?><article class="porumbesti-person"><div class="porumbesti-person-photo"><?php if ( $s['photo']['url'] ) { echo Group_Control_Image_Size::get_attachment_image_html( $s, 'photo', 'photo' ); } else { echo '<span>◎</span>'; } ?></div><div class="porumbesti-person-body"><span class="porumbesti-tag"><?php echo esc_html( $s['role'] ); ?></span><h2><?php echo esc_html( $s['name'] ); ?></h2><p class="porumbesti-muted"><?php echo esc_html( $s['subtitle'] ); ?></p><?php if ( ! empty( $s['bio'] ) ) : ?><div class="porumbesti-person-bio"><?php echo wp_kses_post( $s['bio'] ); ?></div><?php endif; ?><div class="porumbesti-detail-list"><?php if ( $s['phone'] ) : ?><a href="tel:<?php echo esc_attr( preg_replace( '/\D+/', '', $s['phone'] ) ); ?>"><b>TEL</b><span><?php echo esc_html( $s['phone'] ); ?></span></a><?php endif; ?><?php if ( $s['email'] ) : ?><a href="mailto:<?php echo esc_attr( antispambot( $s['email'] ) ); ?>"><b>@</b><span><?php echo esc_html( antispambot( $s['email'] ) ); ?></span></a><?php endif; ?><?php if ( $s['office'] ) : ?><div><b>ORĂ</b><span><?php echo esc_html( $s['office'] ); ?></span></div><?php endif; ?></div></div></article><?php }
}

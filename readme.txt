...
= 1.0.18 =
* Restores the complete official Romanian and Hungarian mayor welcome messages inside the modern homepage message blocks.
* Keeps the original source-derived paragraphs together while preventing leadership markup from being duplicated into the greeting.
* Adds regression coverage for the complete homepage welcome-message content.
...
* Resets Elementor's inherited padding on icon-only header, search and floating controls so their glyphs remain geometrically centered.
* Adds a visual control-alignment fixture and regression assertions for the corrected UI geometry.

= 1.0.15 =
* Extends the homepage content gutter to intermediate desktop widths so the deployed layout keeps the same 24-pixel shell as the local design.

= 1.0.14 =
* Keeps the accessibility panel above the complete floating control stack, within the mobile viewport, and closes it with Escape or an outside click.
* Restores the local design's tablet and mobile side spacing for homepage headings, cards and content blocks.
* Removes desktop horizontal overflow caused by off-screen hidden submenu trees.
